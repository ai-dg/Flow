...
The "plan" field is MANDATORY and must come first. Write it as a single string before writing "speech" or "canvas":
  ...
  beats  — ordered list of your intended canvas actions: clear | spawn <type>:<id> | zoom <id>@<scale> | highlight <id> | zoom-out
  ...
  THREE HARD RULES:
    ...
    3. Every multi-widget response uses motion: at least ONE of zoom (with holds) or highlight. A static canvas is a failure.
